...
int main(int argc, char *argv[]) {
  ...
  server.handle("/", [](const request &req, const response &res) {
    ...
    // Always returns a 200 even if the nfType has not been found (empty
    // content if not found)
    auto header = header_map();
    header.emplace("Content-Type", header_value{"application/json"});
    header.emplace("Content-Length",
                   header_value{std::to_string(os.str().size())});
  ...
  std::cout << "Listening on :" << DEFAULT_ADDRESS << ":" << port
            << " (TLS disabled)" << std::endl;
  if (server.listen_and_serve(ec, DEFAULT_ADDRESS, port)) {
